...
_auto_complete() {
    ...
    then
      ...
      then
        ...
        if [ -f ${HOME}/.govopsrc ];
        then
          chk=$(cat ${HOME}/.govopsrc | grep "JIRACLIENT" >/dev/null 2>&1; echo $?)
          if [ $chk -ne 0 ];
          then
            echo 'eval "$(_JIRACLIENT_COMPLETE=zsh_source jiraclient)"' >> ${HOME}/.govopsrc
          fi
        else
          echo 'eval "$(_JIRACLIENT_COMPLETE=zsh_source jiraclient)"' >> ${HOME}/.govopsrc
        fi
      ...
      if [ -f ${HOME}/.govopsrc ];
      then
        chk=$(cat ${HOME}/.govopsrc | grep "JIRACLIENT_COMPLETE" >/dev/null 2>&1; echo $?)
        if [ $chk -ne 0 ];
        then
          echo 'eval "$(_JIRACLIENT_COMPLETE=bash_source jiraclient)"' >> ${HOME}/.govopsrc
        fi
      else
        echo 'eval "$(_JIRACLIENT_COMPLETE=bash_source jiraclient)"' >> ${HOME}/.govopsrc
      fi
