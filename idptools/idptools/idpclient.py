import sys, os, string, random, urllib
...
class IDPclient:
    ...
    def add_tenant(self, tenant, tenant_type):
        ...
        URL = self.idp + '/SAAS/jersey/manager/api/tenants/tenant'

        HEADERS = {
                'Content-Type': 'application/vnd.vmware.horizon.manager.tenants.tenant+json',
                'Accept': 'application/vnd.vmware.horizon.manager.tenants.tenant+json',
                'Authorization': 'Bearer %s' % self.access_token
                }

        RESP = requests.post(URL, headers=HEADERS, data=json.dumps(TENANT_SPEC))
        return RESP

    def delete_tenant(self, tenant):

        URL = self.idp + '/SAAS/jersey/manager/api/tenants/tenant/' + tenant
        HEADERS = {
                'Content-Type': 'application/vnd.vmware.horizon.manager.tenants.tenant+json',
                'Accept': 'application/vnd.vmware.horizon.manager.tenants.tenant+json',
                'Authorization': 'Bearer %s' % self.access_token
    ...
    def delete_user(self, username, uid):
        ...
        if RESP.status_code != 204:
            Log.critical(f'failed to remove user {username}: ' + RESP.text)

        return RESP.status_code 


    def add_user(self, username, firstname, lastname, email, tenant):
        ...
        # here is the path of the URL
        url = self.idp + '/SAAS/jersey/manager/api/scim/Users?sendMail=false'
        # This is the header we add to the request
        headers = {'Authorization': 'Bearer %s' % self.access_token}
        try:
            # We make the response here
            resp = requests.post(url, headers=headers, data=json.dumps(USER_SPEC))
            # We actually print the whole response
            Log.json(json.dumps(resp.json(), indent=2, sort_keys=True))
        except:
            Log.critical(f'failed to add user {username} to tenant {tenant}')

        # Save the otaLink, this is where it fails since all it's trying to do here is parse the json we already have
        try:
            filename = '/tmp/otaLink_' + username
            otaLink = resp.json()["urn:scim:schemas:extension:workspace:1.0"]["otaLink"]
            f = open(filename, "w+")
            f.write(otaLink)
            f.close
            Log.info('User otaLink file is created: ' + filename)
        except:
            Log.critical('Failed to create otaLink file: ' + filename)
        return otaLink

    def add_admin(self, username, email, tenant):
        ...
        URL = self.idp + '/SAAS/jersey/manager/api/tenants/tenant/' + tenant + '/admin?sendEmail=false'
        HEADERS = {
                'Content-Type': 'application/vnd.vmware.horizon.manager.tenants.tenant.admin+json',
                'Accept': 'application/vnd.vmware.horizon.manager.tenants.tenant.admin+json',
    ...
    def getUserID(self, username):
        ...
        q = urllib.parse.urlencode({'filter': 'userName eq "%s"' % username})
        url = self.idp + '/SAAS/jersey/manager/api/scim/Users?' + q
        headers = {'Authorization': 'Bearer %s' % self.access_token}
        resp = requests.get(url, headers=headers)
        try:
            userid = resp.json()['Resources'][0]['id'] 
        except IndexError:
            userid = None 
        return userid
...
def idp_setup(url, client_id, secret, user_profile='default', overwrite=False):
    CONFIG = Config('idptools')
    IDP = url
    if user_profile not in CONFIG.PROFILES:
        CONFIG.create_profile(user_profile)
