...
def recursive_parse(command, parent_data, stop_condition=None):
    ...
    if parsed_data == -1 or (
        not parsed_data["args"] and
        not parsed_data["options"] and
        not parsed_data["subcommands"] and
        "description" not in parsed_data):
        return -1
    ...
    for subcommand in parsed_data["subcommands"].keys():
        ...
        result = recursive_parse(new_command, current_data, stop_condition)
        if result == -1:
            subcommands_to_remove.append(subcommand)
    for subcommand in subcommands_to_remove:
        parsed_data["subcommands"].pop(subcommand, None)
...
for top_level_command in top_level_commands:
    ...
    result = recursive_parse(top_level_command, initial_data, stop_condition="--cmd")
    if result == -1:
        # continue to next top-level command on the list
        continue
