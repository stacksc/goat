...
def show(ctx, target):
    ...
    if target == 'buckets':
        s3show('buckets', aws_profile_name)
    if target == 'instances':
        ec2show('instances', aws_profile_name, display=True)
    if target == 'public_ips':
        ec2show('public_ips', aws_profile_name, display=True)
    if target == 'regions':
        ec2show('regions', aws_profile_name, display=True)
    if target == 'all_cached':
        ec2show('all', aws_profile_name, display=True)
