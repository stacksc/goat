...
@click.option('-n', '--number-of-hosts', 'host_num', help='number of ESXi hosts to deploy in SDDC', required=False, default='1')
@click.option('-p', '--provider', help='cloud provider to use', type=click.Choice(['AWS', 'ZEROCLOUD']), default='AWS')
@click.option('-a', '--account', help='AWS account to use for the deployment', required=False, default=None)
@click.option('-m', '--customer-cidr', 'net_cus', help='subnet to use for management network', required=False, default=None)
@click.option('-v', '--vpc-cidr', 'net_vpc', help='subnet to use for VPC network', required=False, default='10.2.0.0/16')
@click.option('-t', '--sddc-type', 'sddc_type', help='type of SDDC to deploy', required=False, default='1NODE')
@click.option('-d', '--deployment-type', 'deployment_type', help='single-region or multi-region (stretched) deployment', required=False, type=click.Choice(['SingleAZ', 'MultiAZ']), default='SingleAZ')
@click.option('-s', '--size', 'sddc_size', help='size of the appliances (vCenter, NSX) to deploy', required=False, type=click.Choice(['MEDIUM', 'LARGE']), default='MEDIUM')
@click.option('-i', '--instance-type', 'host_type', help='type of AWS hardware instance to use for deploying ESXi', required=False, default='i3.metal')
@click.option('-r', '--region', help='target cloud provider region to deploy the SDDC to', required=False, type=click.Choice(['west', 'east']), default='west')
@click.pass_context
def sddc_create(ctx, target_org, sddc_name, host_num, provider, account, net_cus, net_vpc, sddc_type, deployment_type, sddc_size, host_type, region):
    AUTH, PROFILE = get_operator_context(ctx)
    VMC = vmc(AUTH, PROFILE)
    RESULT = VMC.sddc_create(target_org, sddc_name, host_num, provider, account, net_cus, net_vpc, sddc_type, deployment_type, sddc_size, host_type, region)
...
@click.pass_context
def sddc_delete(ctx, target_org, sddc_id, force, raw):
    AUTH, PROFILE = get_operator_context(ctx)
    VMC = vmc(AUTH, PROFILE)
    RESULT = VMC.org_sddc_delete(target_org, sddc_id, force, raw)
    Log.info(json.dumps(RESULT, indent=2))
...
@click.option('-r', '--raw', 'raw', help="display raw json; dont reduce output", is_flag=True, required=False, default=False)
@click.pass_context
def sddc_show_details(ctx, sddc_id, raw=False):
    AUTH, PROFILE = get_operator_context(ctx)
    aws_ready = False
    version_only = False
    VMC = vmc(AUTH, PROFILE)
...
@click.option('-r', '--raw', 'raw', help="display raw json; dont reduce output", is_flag=True, required=False, default=False)
@click.pass_context
def sddc_show_all(ctx, auth_org, aws_ready, raw=False):
    AUTH, PROFILE = get_operator_context(ctx)
    VMC = vmc(AUTH, PROFILE)
    RESULT = VMC.show_sddc_all(aws_ready, raw)
    Log.info(json.dumps(RESULT, indent=2))
