...
class vmc():
    ...
    def sddc_create(self, target_org, sddc_name, host_num='1', provider='AWS', account=None, net_cus=None, net_vpc='10.2.0.0/16', 
        ...
        ORG_ID = self.get_org_id(target_org)
        REGION_ID = self.get_region_id(region)
        ORG_PROPERTIES = self.CSP.org_property_list(auth_org_name='operator', target_org_id=ORG_ID, user_profile=self.USER_PROFILE)
        try:
    ...
    def org_sddc_delete(self, target_org, sddc_id, force=False, raw=False):
        ORG_ID = self.get_org_id(target_org)
        SDDC_FACTS = self.CSP.org_show_sddc(target_org, sddc_id, ORG_ID, user_profile=self.USER_PROFILE)
        ...
        URL = f"{self.VMC_URL}/vmc/api/orgs/{ORG_ID}/sddcs/{sddc_id}"
        if force:
            URL = URL + "?force=true"
        RESULT =  curl2.delete(
            url=URL,
    ...
    def show_sddc_all(self, aws_ready=False, raw=False):
        RESULT = curl2.get(
