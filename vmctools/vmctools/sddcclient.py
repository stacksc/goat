...
class sddc():
    ...
    def nsx_consolidated_status(self, raw):
        SDDC = self.show_sddc()
        REDUCE = { 'name':None, 'id':None}
        RESULT = reduce_json(SDDC, REDUCE)
        RESULT['nsx'] = []
    ...
    def nsx_vpn_session_states(self, raw):
        SDDC = self.show_sddc()
        REDUCE = { 'name':None, 'id':None}
        RESULT = reduce_json(SDDC, REDUCE)
        RESULT['nsx'] = []
        ...
        for ID in DATA:
            ID = ID['id']
            RETURN = curl.get(
                url = f"{NSX_ENDPOINT}/api/v1/vpn/ipsec/sessions/{ID}/statistics",
                headers = {
                    'Content-type': 'application/json',
                    'Accept': 'application/json',
                    'csp-auth-token': self.VMC.ACCESS_TOKEN
        ...
        REDUCE = {'display_name':None, 'ipsec_vpn_session_id':None, 'ike_status': {'ike_session_state': None, 'fail_reason': None}}
        DATA = reduce_json(RESULT['nsx'], REDUCE)
        return DATA
