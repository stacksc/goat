...
def failover_edge(ctx, sddc_id, ticket, raw):
    ...
    _get_consolidated_status(SDDC, raw)
    ...
    while not FINISHED and max_loop < 5:
        max_loop = max_loop + 1
        try:
            seconds = 30
            wait_with_message(seconds)
...
@nsx_show.command('vpn-states', help='show the VPN session states for NSX in a specific SDDC', context_settings={'help_option_names':['-h','--help']})
@click.argument('sddc_id', required=False, default=None)
@click.option('-r', '--raw', 'raw', help="display raw json; don't reduce output", is_flag=True, required=False, default=False)
@click.pass_context
def nsx_vpn_endpoints(ctx, sddc_id, raw):
    AUTH, PROFILE = get_operator_context(ctx)
    aws_ready = False
    version_only = False
    if sddc_id is None:
    ...
    SDDC = sddc(sddc_id, AUTH, PROFILE, None)
    RESULT = SDDC.nsx_vpn_session_states(raw)
    DICT = {}

    for I in RESULT:
        DISPLAYNAME = I['display_name']
        ID = I['ipsec_vpn_session_id']
        STATUS = I['ike_status']['ike_session_state']
        FAILREASON = I['ike_status']['fail_reason']
        if not FAILREASON:
            FAILREASON = 'N/A'
        DICT[ID] = {'id': ID, 'display_name': DISPLAYNAME, 'ike_session_state': STATUS, 'fail_reason': FAILREASON}
...
@nsx_show.command('bgp-status', help='show the consolidated vpn bgp status for NSX in a specific SDDC', context_settings={'help_option_names':['-h','--help']})
@click.argument('sddc_id', required=False, default=None)
@click.option('-r', '--raw', 'raw', help="display raw json; don't reduce output", is_flag=True, required=False, default=False)
@click.pass_context
def nsx_consolidated_status(ctx, sddc_id, raw):
    AUTH, PROFILE = get_operator_context(ctx)
    aws_ready = False
    version_only = False
    if sddc_id is None:
...
def _get_consolidated_status(SDDC, raw):

    RESULT = SDDC.nsx_consolidated_status(raw)
    NSX = {}
    DATADICT = {}
    if RESULT != []:
...
def nsx_node_details(ctx, sddc_id, raw, ticket):
    ...
    if RESULT == []:
        ...
        PATTERNS = ['id', 'type', 'display_name', 'ip_address']
        for I in filter_text(RESULT['data']['params']['SCRIPTDATA']['data'].replace(",","\n")).split('\n'):
            line = listToStringWithoutBrackets(I).replace('"','').split(':')
            k = line[0].strip()
            v = line[1].strip()
...
def _parse_edge_data(RESULT):

    NSX = {}
    PATTERNS = ["display_name", "sddc_state", "sddc_version", "org_type", "resource_type", "maintenance_mode", "ip_addresses", "node_display_name", "overall_status", "vm_host", "host_node_deployment_status"]
    for I in filter_text(RESULT['data']['params']['SCRIPTDATA']['data'].replace(",","\n")).split('\n'):
        line = listToStringWithoutBrackets(I).replace('"','').split(':')
        for D in PATTERNS:
            if D == line[0].strip():
                m = line[-1:]
                NSX.update({line[0].strip(): m})

    if NSX:
        Log.info(f"\n{tabulate(NSX, headers='keys', tablefmt='rst')}")
    else:
        Log.warn('there are 0 defined active edges found')
        return None
